...
if(isset($_POST['add_produitt'])){
   ...
   $insert_query = mysqli_query($conn, "INSERT INTO `protection`(nom, image, prix) VALUES('$p_name', '$p_image', '$p_price')") or die('query failed');
...
if(isset($_GET['delete'])){
  ...
  $delete_query = mysqli_query($conn, "DELETE FROM `protection` WHERE id = $delete_id ") or die('query failed');
  if($delete_query){
...
if(isset($_POST['update_product'])){
  ...
  $update_query = mysqli_query($conn, "UPDATE `protection` SET  nom = '$update_p_nom', image = '$update_p_image' , prix = '$update_p_prix' WHERE id = '$update_p_id'");
...
<body>
    ...
    <section id="product1" class="section-1">
        ...
        <h2>PROTECTION OCULAIRE</h2>
        <P>la marque MEDOP</P>
        <div class="pro-container">
        <?php
         
         $select_products = mysqli_query($conn, "SELECT * FROM `protection`");
         if(mysqli_num_rows($select_products) > 0){
            while($row = mysqli_fetch_assoc($select_products)){
        ?>
            <div class="pro">
                <img src="img/protection/<?php echo $row['image']; ?>">
                <div class="des">
                    <h5><?php echo $row['nom']; ?></h5>
                    <h4>$<?php echo $row['prix']; ?></h4>
                    <p>demande de devis</p>
                </div>
                <a href="protection.php?edit=<?php echo $row['id']; ?>"><i class="fa-solid fa-pen-to-square" id="carttt"></i></a>
                <a href="protection.php?delete=<?php echo $row['id']; ?>" class="delete-btn" onclick="return confirm('are your sure you want to delete this?');"><i class="fa-solid fa-trash" id="cartt"></i></a>
   ...
   if(isset($_GET['edit'])){
      $edit_id = $_GET['edit'];
      $edit_query = mysqli_query($conn, "SELECT * FROM `protection` WHERE id = $edit_id");
