...
if(isset($_POST['add_produitt'])){
   ...
   $insert_query = mysqli_query($conn, "INSERT INTO `chaussure`(nom, image, prix) VALUES('$p_name', '$p_image', '$p_price')") or die('query failed');
...
if(isset($_GET['delete'])){
  ...
  $delete_query = mysqli_query($conn, "DELETE FROM `chaussure` WHERE id = $delete_id ") or die('query failed');
  if($delete_query){
     header('location:chaussure_securite.php');
...
if(isset($_POST['update_product'])){
  ...
  $update_query = mysqli_query($conn, "UPDATE `chaussure` SET detail ='$update_p_detail', nom = '$update_p_nom', image = '$update_p_image' , prix = '$update_p_prix' WHERE id = '$update_p_id'");
...
if(isset($_GET['cart'])){
  $edit_id = $_GET['cart'];
  $edit_query = mysqli_query($conn, "SELECT * FROM `chaussure` WHERE id = $edit_id");
  echo"<script>alert( $edit_query)</script>";
  if(mysqli_num_rows($edit_query) > 0){
     while($fetch_edit = mysqli_fetch_assoc($edit_query)){

$name =$fetch_edit['nom'];
$prix = $fetch_edit['prix'];
$p_image = $fetch_edi['p_image']['name'];
$p_image_tmp_name = $fetch_edi['p_image']['tmp_name'];
$p_image_folder = 'uploaded_img/'.$p_image;

$insert_query = mysqli_query($conn, "INSERT INTO `cart`(nom, image, prix) VALUES('$name', '$image', '$prix')") or die('query failed');
...
<head>
    ...
    <section id="product1" class="section-1">
        ...
        <h2>CHAUSSURE DE SECURITE</h2>
        <P>PARDES</P>
        <div class="pro-container">
        <?php
         
         $select_products = mysqli_query($conn, "SELECT * FROM `chaussure`");
        ...
        ?>
            <div class="pro" >
                ...
                <div>
                <img src="img/chaussure/<?php echo $row['image']; ?>" >
                </div>
                <div class="des" >
                ...
                <a href="chaussure_securite.php?edit=<?php echo $row['id']; ?>"><i class="fa-solid fa-pen-to-square" id="carttt"></i></a>
                <a href="chaussure_securite.php?delete=<?php echo $row['id']; ?>" class="delete-btn" onclick="return confirm('are your sure you want to delete this?');"><i class="fa-solid fa-trash" id="cartt"></i></a>
                <a href="cart.php?cart=<?php echo $row['id']; ?>"><i class="fa-solid fa-cart-plus " id="cart"></i></a>
                </div>
